...
while [ $(ping -c 1 ${MYSQL_PUBLIC_IP} 2>&1 | grep -c " 0% packet loss") -eq 0 ]; do
    sleep 1
done
...
./ansible/playbooks/mysql-vm-and-os-v2.yml
...
########################################################################################################################
# ClickHouse VM and OS                                                                                                      #
########################################################################################################################

echo_module_start "ClickHouse VM and OS "
echo_cyan "Setting up ClickHouse VM and OS...\n"
echo_cyan "Executing vagrant...\n"
...
while [ $(ping -c 1 ${CLICKHOUSE_PUBLIC_IP} 2>&1 | grep -c " 0% packet loss") -eq 0 ]; do
    sleep 1
done
...
--user="vagrant" \
--private-key="${SSH_PRIVATE_KEY_FILE}" \
./ansible/playbooks/clickhouse-vm-and-os-v2.yml

if [ "$?" -gt 0 ]; then
    echo_red "\nansible-playbook for ClickHouse VM and OS failed!"
    exit 1
else
    echo_green "\nansible-playbook for ClickHouse VM and OS succeeded!"
fi
